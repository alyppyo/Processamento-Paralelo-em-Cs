...
namespace Ex6
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            while (true)
            {
                Conta atual = clientes[rand.Next(clientes.Count)];

                int proximaOperacao = rand.Next(3);
                decimal valor = ValorDecimalAleatorioAte(100);
                switch (proximaOperacao)
                {
                    case 0:
                        await atual.DepositarAsync(valor);
                        Console.WriteLine($"+ Depositado {valor} na conta de {atual.Usuario}. Saldo: {atual.Saldo}.");
                        break;
                    case 1:
                        await atual.SacarAsync(valor);
                        Console.WriteLine($"- Sacado {valor} da conta de {atual.Usuario}. Saldo: {atual.Saldo}.");
                        break;
                    case 2:
                        Conta destino = clientes[rand.Next(clientes.Count)];
                        if (atual != destino)
                        {
                            await atual.TransferirAsync(destino, valor);
                            Console.WriteLine($"> Transferido {valor} da conta de {atual.Usuario} para a conta de {destino.Usuario}.\n> Saldo ({atual.Usuario}): {atual.Saldo}\n> Saldo ({destino.Usuario}: {destino.Saldo}).");
                        }
                        break;
                }
            }
        ...
        static decimal ValorDecimalAleatorioAte(int valorMaximo)
        {
            return (decimal)Math.Round(new Random().NextDouble() * valorMaximo, 2);
        }
    }
}
